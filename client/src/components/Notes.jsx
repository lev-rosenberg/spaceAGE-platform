...
export function Notes({ handleReturnToFullSize }) {
  ...
  useEffect(() => {
    // Initialize local notes if they are empty
    if (Object.keys(localSliderNotes).length === 0) {
      dispatch({ type: 'SET_LOCATION_SLIDER_NOTES', payload: serverSideSliderNotes });
    }
    if (Object.keys(localTextNotes).length === 0) {
    ...
    if (!clicked && locationCoords.length > 0) {
      dispatch({ type: 'SET_CLICKED', payload: locationCoords[0].name });
    }
  }, [dispatch, serverSideSliderNotes, serverSideTextNotes, clicked, locationCoords, localSliderNotes, localTextNotes]);

  function handleButtonClick(location) {
    if (location.name === clicked) return; // Prevent duplicate clicks
    handleReturnToFullSize(location.name);
  }
  ...
  return (
    <div className={`${styles.bwSection} ${styles.notes}`}>
      ...
      <form disabled={false}>
        <div className='flex flex-col gap-2 mb-3'>
          {Object.keys(localSliderNotes).length !== 0 && clicked in localSliderNotes && Object.keys(localSliderNotes[clicked]).map((slider, i) => (
            <SliderInput
              key={i}
              value={localSliderNotes[clicked][slider]}
              label={slider}
              handleChange={(e) => {
                handleLocalChange('SET_LOCATION_SLIDER_NOTES', {
                  ...localSliderNotes,
                  [clicked]: {
                    ...localSliderNotes[clicked],
        ...
        <TextInput
          className='w-full h-32'
          value={localTextNotes[clicked] || ''} // Provide a default value to avoid undefined
          label='localSliderNotesLabel'
          placeholder={`Notes about ${clicked}`}
          area
          handleChange={(e) => {
            handleLocalChange('SET_LOCATION_TEXT_NOTES', {
              ...localTextNotes,
